...
class AnnuaireElevageNonConventionnel {
    ...
    AnnuaireElevageNonConventionnel(annee){
        
        headers = ElevageNonConventionnel.executeQuery("select distinct  espece.libelle from ElevageNonConventionnel where annee = :a   order by espece.libelle asc", [a: annee])
        
        data = ElevageNonConventionnel.executeQuery("select departement, commune, espece.libelle, sum(effectif) from ElevageNonConventionnel where annee = :a  group by departement, commune, espece.libelle order by departement asc", [a: annee])
        
        en_tete = ["Departement","Commune"] + ["Espece/Animaux"]*headers.size()
        
        workbook = new XSSFWorkbook();
        //Create a blank sheet
        sheet = workbook.createSheet("Annuaire Statistique Elevage Non Conventionnel")  
    }
